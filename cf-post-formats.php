...
function cfpf_add_meta_boxes($post_type) {
	if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
		...
		if( is_rtl() ){
			wp_enqueue_style('cf-post-formats', cfpf_base_url().'css/admin-rtl.css', array(), CFPF_VERSION, 'screen');
		}
		else{
...
function cfpf_format_gallery_save_post( $post_id ) {
	if (!defined('XMLRPC_REQUEST')) {
		...
		// special for checkbox input only
		if (isset($_POST['_format_gallery_show_lighbox'])) {
			update_post_meta( $post_id, '_format_gallery_show_lighbox', esc_html($_POST['_format_gallery_show_lighbox']) );
		}
		else{
			delete_post_meta( $post_id, '_format_gallery_show_lighbox' );
		}
	}
}
// action added in cfpf_admin_init()
...
function cfpf_post_gallery_lightbox() {
	$post = get_post();
	$value = get_post_meta($post->ID, '_format_gallery_show_lighbox', true);
	switch ($value) {
		case 'on':
			$value = 1;
		break;
